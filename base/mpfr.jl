...
doc"""
    big(x::AbstractString)

Return a `BigFloat` representation of `x`.
"""
big(x::AbstractString) = parse(BigFloat, x)
...
Create a `BigFloat` representation of `x` with given rounding mode.
"""
